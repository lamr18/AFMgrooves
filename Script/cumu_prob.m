...
for i=1:length(perp_line_pixels) % circle through the perpendicular profile lines
    ...
    symm_tab=rmmissing(symm_tab); % remove NaN lines
    symm_tab(~symm_tab.width,:) = []; %remove 0 lines
    nosymm_tab=rmmissing(nosymm_tab); % remove NaN lines
    nosymm_tab(~nosymm_tab.width,:) = []; %remove 0 lines
end
...
colorlines(sum(isnan(colorlines), 2) == 1, :) = [];
