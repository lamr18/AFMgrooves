...
%READ_HEADER Allows for an initial read of the file and partition into header 
%and data. It returns a list with the length of the header and an unformatted
%header.
...
all_lines = readlines(filename); % read all the lines in the file
lines_header = find(contains(all_lines,'#!'))-1; % tells us where the data section starts and how many lines there are in the header
lines_header=lines_header(1);%there may be several hits, so the first instance is our delimiter
headerlines = all_lines(1:(lines_header)); % truncate the data beyond the end of the header

% Count characters in the header
len_header = 0; 
for k = 1:lines_header
    len_header = len_header + strlength(headerlines(k))+2;
end

%Format the header so that it is more easily navigable
from = find(startsWith(headerlines,'[')); % list index of all header sections (start with '[')
